...
def arguments():
    ...
    parser.add_argument("--evaluation-modes", nargs="+", choices=("all", "task_specific", "learned_gates"), default=["all", "task_specific", "learned_gates"])
    ...
    parser.add_argument("--adapter-eval-mode", choices=("all", "task_specific", "learned_gates"), default="all")
...
def variants(args):
    return list(product(args.ranks, args.evaluation_modes, args.penalty_types))


def variant_name(rank, evaluation_mode, penalty_type):
    return f"rank_{rank}_{evaluation_mode}_{penalty_type}"
...
def summary(path, name, rank, evaluation_mode, penalty_type):
    with path.open(newline="") as handle:
...
def main():
    ...
    for rank, evaluation_mode, penalty_type in variants(args):
        name = variant_name(rank, evaluation_mode, penalty_type)
        ...
        run_args.orthogonal_penalty_weight = args.orthogonal_weight
